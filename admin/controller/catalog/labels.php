<?php
class ControllerCatalogLabels extends Controller {
	private $error = array();

	public function index() {
		$this->load->language('catalog/lebel');
	...
	protected function getList() {
		...
		$data['sort_name'] = $this->url->link('catalog/label', 'token=' . $this->session->data['token'] . '&sort=name' . $url, true);
		$data['sort_sort_order'] = $this->url->link('catalog/label', 'token=' . $this->session->data['token'] . '&sort=sort_order' . $url, true);
	...
	protected function getForm() {
		...
		if (isset($this->request->post['label_store'])) {
			$data['label_store'] = $this->request->post['label_store'];
		} elseif (isset($this->request->get['label_id'])) {
			$data['label_store'] = $this->model_catalog_label->getLabelStores($this->request->get['label_id']);
		} else {
			$data['label_store'] = array(0);
		}

		if (isset($this->request->post['keyword'])) {
			$data['keyword'] = $this->request->post['keyword'];
		} elseif (!empty($label_info)) {
			$data['keyword'] = $label_info['keyword'];
		} else {
			$data['keyword'] = '';
		}
	...
	protected function validateForm() {
		...
		if (utf8_strlen($this->request->post['keyword']) > 0) {
			...
			if ($url_alias_info && isset($this->request->get['label_id']) && $url_alias_info['query'] != 'label_id=' . $this->request->get['label_id']) {
				$this->error['keyword'] = sprintf($this->language->get('error_keyword'));
			}

			if ($url_alias_info && !isset($this->request->get['label_id'])) {
				$this->error['keyword'] = sprintf($this->language->get('error_keyword'));
			}
		}
